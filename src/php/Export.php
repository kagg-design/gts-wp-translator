 ...
 * Otherwise, we have a fatal error "Cannot redeclare...".
 */
class Export {
	...
	public function export_wp( $args = [] ) {
		...
		// We do not set headers.

		if ( 'all' !== $args['content'] && post_type_exists( $args['content'] ) ) {
			$ptype = get_post_type_object( $args['content'] );

			if ( ! $ptype || ! $ptype->can_export ) {
				$args['content'] = 'post';
			}
	...
	private function wxr_nav_menu_terms() {
		...
		foreach ( $nav_menus as $menu ) {
			echo "\t<wp:term>";
			echo '<wp:term_id>' . $menu->term_id . '</wp:term_id>';
	 ...
	 * Filter post meta.
	 *
	 * @param bool   $return_me Filter flag.
